    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result, function (key, item) {
                ...
                html += '<td><a href="#" onclick="return getbyID(' + item.ID + ')" class="btn btn-primary"> Edit</a>  <a href="#" onclick="Delete(' + item.ID + ')">Delete</a></td>';
                html += '</tr>';
            });
            $('.tbody').html(html);
        },
...
function Delete(ID) {
    ...
    if (ans) {
        $.ajax({
            url: "/Company/DeleteMeetingOne/" + ID,
            type: "POST",
            contentType: "application/json;charset=UTF-8",
            dataType: "json",
            success: function (result) {
